...
@section('title', 'Pilóta szerkesztése')
...
<div class="py-12">
    <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                <h2 class="text-2xl font-bold mb-6">Pilóta szerkesztése</h2>
                ...
                <form method="POST" action="{{ route('drivers.update', $driver) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="sex" name="sex" required
                            ...
                            <option value="F" {{ old('sex', $driver->sex) == 'F' ? 'selected' : '' }}>Férfi</option>
                            <option value="N" {{ old('sex', $driver->sex) == 'N' ? 'selected' : '' }}>Nő</option>
                        </select>
                        @error('sex')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <input id="birth_date" type="date" name="birth_date" value="{{ old('birth_date', $driver->birth_date) }}" required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        @error('birth_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <input id="country" type="text" name="country" value="{{ old('country', $driver->country) }}" required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        @error('country')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
